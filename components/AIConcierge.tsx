...
const AIConcierge: React.FC = () => {
  ...
  useEffect(() => {
    if (isOpen) {
      // Small timeout to allow the panel animation to begin/complete
  ...
  return (
    <>
      ...
      <button
        ...
        type="button"
        aria-label="Open Imperial Concierge"
          ...
          <div ref={scrollRef} className="flex-1 overflow-y-auto p-8 space-y-6 scrollbar-hide">
            ...
            {isTyping && (
              <div className="flex justify-start">
                <div className="bg-[var(--color-background)] text-[var(--color-primary)] p-5 rounded-3xl rounded-tl-none border border-[var(--color-primary)]/5 flex gap-1.5 items-center">
                   <div className="w-1.5 h-1.5 bg-[var(--color-primary)]/40 rounded-full animate-bounce [animation-delay:-0.3s]" />
                   <div className="w-1.5 h-1.5 bg-[var(--color-primary)]/40 rounded-full animate-bounce [animation-delay:-0.15s]" />
                   <div className="w-1.5 h-1.5 bg-[var(--color-primary)]/40 rounded-full animate-bounce" />
                </div>
              </div>
            )}
          </div>
