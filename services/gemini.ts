...
/**
 * Centralized AI instance factory to ensure consistent API key usage and robustness.
 */
export const getGenAI = () => new GoogleGenAI(API_KEY);

/**
 * Sanitizes user input to prevent prompt injection by escaping quotes and stripping malicious keywords.
 */
export const sanitizePromptInput = (input: string | undefined | null, maxLength: number = 500): string => {
  if (!input) return "";
  return input
    .slice(0, maxLength)
    .replace(/"/g, '\\"')
    .replace(/SYSTEM:/gi, "[sanitized]")
    .replace(/IGNORE PREVIOUS/gi, "[sanitized]")
    .replace(/USER INSTRUCTIONS:/gi, "[sanitized]");
};
