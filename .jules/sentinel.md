...
## 2026-02-02 - [Sanitizing User Input for Prompt Injection Protection]
**Vulnerability:** User-controlled strings (queries, instructions, suggestions) were directly interpolated into AI prompts without sanitization, allowing potential prompt injection.
**Learning:** Robust protection against prompt injection in this codebase requires escaping backslashes and double quotes and limiting input length to 500 characters to prevent instruction overrides.
**Prevention:** Always use `sanitizePromptInput` on any user-provided data before including it in a prompt string.
